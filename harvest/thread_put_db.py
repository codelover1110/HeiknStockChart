...
def get_candles(stock, interval, candle_type):
...
def get_tables_name():
    ...
    masterdb = mongoclient["backtesting_tables"]
    backtesting_db = mongoclient["backtesting_1_hour"]

    for coll in backtesting_db.list_collection_names():
        stock_name = coll
...
class SymbolCandleThread(object):
    ...
    def set_params(self, symbol, interval):
        ...
        db_name = 'backtest_' + str(interval[0]) + '_' + interval[1]
        masterdb = mongoclient[db_name]
    ...
    def get_candles(self, stock, interval, candle_type):
        ...
        end_date = self.current_date + timedelta(days=30)
    ...
    def get_candle_thread_func(self):
        while True:
            ...
            for idx, candle in enumerate(candles):
                ...
                if self.candle_queue.qsize() > 100:
                    time.sleep(0.1)
    ...
    def put_candle_thread_func(self):
        ...
        while True:
            ...
            if not self.candle_queue.empty():
                ...
                #     self.fill_space(candle)

                total_candle_count += 1
                print ('{}: {}: {}: {}'.format(
                        self.symbol, 
                        str(self.interval[0])+'_'+self.interval[1], 
                        total_candle_count, 
                        str(candle['date']))
                )
                self.ob_table.insert_one(candle)
...
if __name__ == "__main__":
    ...
    #         db_name = 'backtest_' + str(interval[0]) + '_' + interval[1]
    #         start_date, end_date = define_start_date(interval)
    #         get_put_candles(symbol, interval[0], interval[1], start_date, end_date, db_name)
